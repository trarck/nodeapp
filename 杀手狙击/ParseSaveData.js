...
function decrypto(task,srcFile,outFile){
    ...
    //base64 decode
    var byteContent=new Buffer(content.toString(),"base64");

    //decrypto logic
    var decipher=createDecipher(cryptoKey,VIKey);

    var plainCnt=decipher.update(byteContent);
    plainCnt=Buffer.concat([plainCnt,decipher.final()]);

    //lzma file
    fs.writeFileSync(outFile+".lzma",plainCnt,"binary");

    task.done();
}

function encrypto(task,srcFile,outFile){
    ...
    if (m!=0){
        var add=16-m;
        var addBuff=new Buffer(add);
        content=Buffer.concat([content,addBuff]);
         
    }

    var cipher=createCipher(cryptoKey,VIKey);

    var cryptoCnt=cipher.update(content);

    cryptoCnt=Buffer.concat([cryptoCnt,cipher.final()]);

    fs.writeFileSync(outFile,cryptoCnt.toString("base64"));
    task.done();
}
...
function createCipher(key,iv){
    ...
    var cipher=crypto.createCipheriv(algorithm, keyBuff,ivBuff);
